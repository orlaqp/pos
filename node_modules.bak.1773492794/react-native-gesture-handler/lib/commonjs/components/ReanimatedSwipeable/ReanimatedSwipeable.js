...
const Swipeable = props => {
  ...
  const animateRow = (0, _react.useCallback)((toValue, velocityX = 0) => {
    'worklet';
  ...
  const panGesture = (0, _react.useMemo)(() => {
    const pan = _.Gesture.Pan().enabled(enabled !== false).hitSlop(hitSlop).enableTrackpadTwoFingerGesture(enableTrackpadTwoFingerGesture).activeOffsetX([-dragOffsetFromRightEdge, dragOffsetFromLeftEdge]).onStart(updateElementWidths).onUpdate(event => {
      userDrag.value = event.translationX;
