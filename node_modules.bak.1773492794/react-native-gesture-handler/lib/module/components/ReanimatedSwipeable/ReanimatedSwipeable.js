...
import { useMemo, useCallback, useImperativeHandle, useState } from 'react';
...
const Swipeable = props => {
  ...
  const tapGesture = useMemo(() => {
    const tap = Gesture.Tap().shouldCancelWhenOutside(true).enabled(shouldEnableTap).onStart(() => {
      if (rowState.value !== 0) {
        close();
      }
    });
  ...
  const panGesture = useMemo(() => {
    const pan = Gesture.Pan().enabled(enabled !== false).hitSlop(hitSlop).enableTrackpadTwoFingerGesture(enableTrackpadTwoFingerGesture).activeOffsetX([-dragOffsetFromRightEdge, dragOffsetFromLeftEdge]).onStart(updateElementWidths).onUpdate(event => {
      userDrag.value = event.translationX;
    ...
    return pan;
  }, [enabled, hitSlop, enableTrackpadTwoFingerGesture, dragOffsetFromRightEdge, dragOffsetFromLeftEdge, updateElementWidths, relationProps, userDrag, rowState, dragStarted, updateAnimatedEvent, onSwipeableOpenStartDrag, onSwipeableCloseStartDrag, handleRelease]);
