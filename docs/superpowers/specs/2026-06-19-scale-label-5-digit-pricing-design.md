...
Support scale labels that encode a 4-digit PLU, one scale/control digit, and 5-digit total price while preserving existing tenants that use the current 4-digit price format.

The target migrated scale label format is:

```text
02 + 4-digit PLU + scale/control digit + 5-digit total price + check digit
```
...
For `EAN13_02_4_PLU_5_PRICE`, parse:

```text
02 + 4-digit PLU + scale/control digit + 5-digit total price + check digit
```

When a tenant is migrated to `EAN13_02_4_PLU_5_PRICE`, the search must try the new EAN-13 profile first. If it does not resolve to a product, it must fall back to the legacy parser so old labels can continue to work during transition.
...
1. Confirm the customer scale emits `02 + 4-digit PLU + scale/control digit + 5-digit total price + check digit`.
2. Confirm product PLUs are stored as the matching 4-digit PLUs. Leading zero normalization may continue to support labels and stored PLUs that differ only by leading zeroes.
3. Update `GlobalSettings.scaleBarcodePriceFormat` for the tenant to `EAN13_02_4_PLU_5_PRICE`.
...
- migrated profile parses `02 + 4-digit PLU + scale/control digit + 5-digit total price + check digit`
...
- Migrated tenants can scan the new 4-digit PLU/5-digit price label format.
